-- 「みんなの式」（任意公開ギャラリー）用のテーブル。
-- Supabase の SQL Editor にそのまま貼り付けて実行してください（既存のテーブルにも追記できます）。
...
create table if not exists public.public_formulas (
  ...
  elements jsonb not null check (jsonb_array_length(elements) between 2 and 8),
  ...
  -- 運営が個別に取り下げるための旗。true の行は誰にも見えなくなる
  hidden boolean not null default false
);

alter table public.public_formulas
